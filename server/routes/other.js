...
module.exports = (app, supabase) => {
  ...
  app.post('/api/homework', auth, async (req, res) => {
    try {
      ...
      if (!student_id || !title) {
        return res.status(400).json({ error: 'Укажите ученика и название' });
      }
  ...
  app.get('/api/feedback/my', auth, async (req, res) => {
    try {
      ...
      console.error('GET /api/feedback/my error:', err);
      res.json([]);
    }
  });
  ...
  app.get('/api/achievements', auth, async (req, res) => {
    try {
      ...
      const stats = {
        meetings_count: bookings?.count || 0,
        messages_count: msgs?.count || 0,
        words_count: words?.count || 0,
        rating_rank: rank,
      ...
      const result = (allAchievements || []).map(a => {
        ...
        return {
          ...
          progressPercent: earned ? 100 : Math.min(100, Math.round((cv / a.condition_value) * 100))
        };
      });
  ...
  app.get('/api/parent/students', auth, async (req, res) => {
    try {
      ...
      console.error('GET /api/parent/students error:', err);
  ...
  app.post('/api/parent/bind', auth, async (req, res) => {
    try {
      ...
      if (existing) {
        return res.status(409).json({ error: 'Этот родитель уже привязан к ученику' });
      }
      ...
      console.error('POST /api/parent/bind error:', err);
      res.status(500).json({ error: 'Не удалось привязать родителя' });
    }
  });
  ...
  app.delete('/api/parent/unbind/:studentId/:parentId', auth, async (req, res) => {
    try {
      ...
      console.error('DELETE /api/parent/unbind error:', err);
      res.status(500).json({ error: 'Не удалось отвязать родителя' });
    }
  });
};

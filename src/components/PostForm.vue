<template>
  <div class="modal-overlay" v-if="show" @click.self="$emit('close')">
    <div class="modal" style="max-width:650px">
      ...
      <button class="btn btn-p w-100 mt-3" @click="submit">{{ editId ? '💾 Сохранить' : 'Опубликовать' }}</button>
...
export default {
  ...
  methods: {
    addItem() { this.form.items.push({ type: 'image', urls: [], files: [] }); },
    removeItem(i) { this.form.items.splice(i, 1); },
    removeItemUrl(i, j) { this.form.items[i].urls.splice(j, 1); },
    ...
    async submit() {
      if (!this.form.title || !this.form.content) return alert('Заполните заголовок и текст');
      try {
...
.modal { background: #fff; border-radius: 16px; padding: 24px; width: 100%; max-height: 85vh; overflow-y: auto; }
.input { width: 100%; padding: 10px; border: 2px solid #e2e8f0; border-radius: 10px; font-family: inherit; font-size: 0.9rem; margin-bottom: 8px; }
.items-section { margin: 12px 0; }
.item-row { display: flex; flex-wrap: wrap; gap: 8px; align-items: center; margin-bottom: 8px; padding: 8px; background: #f8fafc; border-radius: 8px; }
.thumb-grid { display: flex; flex-wrap: wrap; gap: 4px; }
.thumb { width: 50px; height: 50px; object-fit: cover; border-radius: 6px; cursor: pointer; }
.file-box { padding: 4px 8px; background: #e2e8f0; border-radius: 6px; font-size: 0.8rem; }
.btn { display: inline-flex; align-items: center; justify-content: center; gap: 4px; padding: 8px 16px; border-radius: 50px; font-weight: 600; font-size: 0.85rem; cursor: pointer; border: none; font-family: inherit; }

<template>
  <div class="chat-container" :class="{ dark: isDark }" v-if="user?.id">
    ...
    <div class="chat-main empty-chat" v-else>
      <div><i class="fas fa-comments" style="font-size:4rem;opacity:0.3"></i><p>Выберите чат</p></div>
    </div>
  </div>
...
export default {
  ...
  async mounted() {
    ...
    this.socket.on('dm', () => { this.loadDialogs(); });
  ...
  methods: {
    ...
    startChat(u) { this.activeChat = u.id; this.activeChatName = u.username; this.partnerAvatar = u.avatar_url; this.searchQuery = ''; this.searchResults = []; this.showSidebar = false; this.loadDialogs(); },
    openChat(d) { const pid = d.partner_id || d.id; this.activeChat = pid; this.activeChatName = d.username; this.partnerAvatar = d.avatar_url; this.showSidebar = false; },
  }
};
</script>

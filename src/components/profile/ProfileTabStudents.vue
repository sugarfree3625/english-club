<template>
  <div class="students-page">
    ...
    <!-- Секция 1: Привязанные ученики -->
    <div class="section">
      <h3 class="section-title earned">
        <CheckCircle :size="20" /> Привязанные ученики
      </h3>
        ...
        <div v-for="parent in boundParents" :key="parent.id" class="family-card">
          ...
          <div class="family-children">
            <div v-for="child in parent.children" :key="child.id" class="child-row">
              ...
              <button class="unlink-btn" @click="confirmUnlink(child, parent)">
    ...
    <!-- Секция 2: Непривязанные ученики -->
    <div class="section">
      <h3 class="section-title locked">
        <Link :size="20" /> Непривязанные ученики
      </h3>
      <div class="unlinked-list">
        <div v-for="student in unlinkedStudents" :key="student.id" class="unlinked-row">
          ...
          <button class="link-btn" @click="openBindModal(student)">
            <Link :size="14" /> Привязать
          </button>
        </div>
    ...
    <div class="section">
      ...
      <div class="quick-bind">
        <div class="quick-bind-row">
          ...
          <button class="quick-bind-btn" @click="quickBind" :disabled="!selectedQuickParent || !selectedQuickStudent">
            <Link :size="16" /> Привязать
          </button>
        </div>
        <div class="quick-results" v-if="quickParentResults.length || quickStudentResults.length">
          <div v-if="quickParentResults.length" class="quick-dropdown">
            <div v-for="p in quickParentResults" :key="p.id" class="quick-item" :class="{ selected: selectedQuickParent?.id === p.id }" @click="selectQuickParent(p)">
              {{ p.username }}
            </div>
          </div>
          <div v-if="quickStudentResults.length" class="quick-dropdown">
            <div v-for="s in quickStudentResults" :key="s.id" class="quick-item" :class="{ selected: selectedQuickStudent?.id === s.id }" @click="selectQuickStudent(s)">
              {{ s.username }}
            </div>
          </div>
        </div>
      ...
      <div class="modal-card">
        ...
        <div class="modal-list">
          <div v-for="p in parentSearchResults" :key="p.id" class="modal-item" @click="bindStudent(p)">
            ...
            <div>
              <div class="modal-item-name">{{ p.username }}</div>
              <span class="role-badge parent-badge">Родитель</span>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    openBindModal(student) {
      this.bindTargetStudent = student;
      this.showBindModal = true;
      this.parentSearch = '';
      this.searchParentsForBind();
    },
    ...
    confirmUnlink(student, parent) {
      this.unlinkStudent = student;
    ...
    async doUnlink() {
      try {
        await axios.delete(`/api/parent/unbind/${this.unlinkStudent.id}/${this.unlinkParent.id}`);
        this.showUnlinkConfirm = false;
        this.addToast(`${this.unlinkStudent.username} отвязан`, 'info');
        await this.loadData();
      } catch(e) {
        this.addToast('Ошибка отвязки', 'error');
      }
    },

        ...
        <div v-if="selected" class="modal-overlay" @click.self="selected = null">
          <div class="modal-card" :class="selected.rarity || 'bronze'">
            ...
            <div v-if="selected.earned" class="modal-earned">
              <div class="earned-badge"><i class="fas fa-check-circle"></i> Получено</div>
              <div v-if="selected.earned_at" class="earned-date">{{ formatDate(selected.earned_at) }}</div>
            </div>
            <div v-else class="modal-locked">
              ...
              <p class="remaining-text">Осталось: {{ selected.condition_value - (selected.current_value || 0) }} {{ unitLabel(selected.condition_field) }}</p>
              <p class="motivation-text">Продолжай в том же духе! 🚀</p>
            </div>
            <div class="modal-actions">
              <button class="modal-close-btn" @click="selected = null">Закрыть</button>
              <button class="modal-share-btn" @click="shareAchievement"><i class="fas fa-share-alt"></i> Поделиться</button>
            </div>
          </div>
        </div>
      </Transition>
...
export default {
  ...
  setup(props) {
    ...
    const filteredAchievements = computed(() => {
      ...
      if (activeCategory.value !== 'all') list = list.filter(a => a.condition_field === activeCategory.value);
      return list;
    });
    ...
    function rarityLabel(r) { const labels = { platinum:'💎 Платина', gold:'🥇 Золото', silver:'🥈 Серебро', bronze:'🥉 Бронза' }; return labels[r] || '🥉 Бронза'; }
    function unitLabel(f) { const units = { meetings_count:'встреч', messages_count:'сообщений', words_count:'слов', rating_rank:'позиций', account_age_days:'дней' }; return units[f] || ''; }
...
.ach-card { position: relative; padding: 20px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); border-radius: 18px; cursor: pointer; transition: all 0.3s; display: flex; flex-direction: column; gap: 10px; overflow: hidden; }
...
.ach-card.ach-earned { border-color: rgba(16,185,129,0.3); }
...
.ach-desc { font-size: 0.75rem; color: #94a3b8; margin-top: 4px; }
.ach-progress { }
...
.modal-card { background: rgba(15,15,30,0.98); border-radius: 24px; padding: 36px 28px; max-width: 420px; width: 100%; text-align: center; color: #fff; animation: modalPop 0.4s ease; position: relative; overflow: hidden; }

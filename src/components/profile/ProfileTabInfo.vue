  ...
  <div class="info-tab">
    ...
    <div class="card fade-in" v-if="nextMeeting">
      ...
      <div class="meeting-preview">
        ...
        <button class="btn btn-p btn-sm" @click="goToCalendar">Перейти</button>
      </div>
    </div>
    <div class="card fade-in empty-meeting" v-else>
      <h3>📅 Ближайшая встреча</h3>
      <p class="empty-text">Нет запланированных встреч</p>
      <button class="btn btn-p btn-sm" @click="goToCalendar">Записаться</button>
    </div>
    ...
    <div class="card fade-in">
      ...
      <div class="quick-actions">
        <button class="action-btn" @click="goToTab('words')">
          <span class="action-icon">📚</span>
          <span>Словарь</span>
        </button>
        <button class="action-btn" @click="goToTab('myschedule')">
          <span class="action-icon">📅</span>
        ...
        <button class="action-btn" @click="goToTab('achievements')">
          <span class="action-icon">🏆</span>
...
export default {
  ...
  methods: {
    ...
    goToTab(tab) {
      this.$parent.tab = tab;
      // Если есть метод load — вызываем
      const loads = {
        words: 'loadWords',
        myschedule: 'loadMySlots',
        achievements: 'loadAchievements'
      };
      if (loads[tab] && this.$parent[loads[tab]]) {
        this.$parent[loads[tab]]();
      }
    },

  ...
  <div class="card mini-game-card">
    ...
    <div class="wordle-input" v-if="!solved && currentRow < 6">
      <input 
        ...
        autocomplete="off"
      />
      <button @click="checkWord" class="wordle-check-btn" :disabled="input.length !== word.length">
        ✓
      </button>
    </div>
    ...
    displayGuesses() {
      ...
      for (let i = 0; i < 6; i++) {
        if (i < this.guesses.length) {
          ...
          rows.push(Array(this.word.length).fill({ letter: '', status: '' }));
        }
      }
      return rows;
    }
  ...
  methods: {
    async loadWords() {
      try {
        ...
        this.allWords = (data || []).filter(w => w.en && w.en.length >= 4 && w.en.length <= 6);
      } catch(e) {
        // Запасные слова если API недоступен
        this.allWords = [
    ...
    resetGame() {
      ...
      this.currentRow = 0;
      this.$nextTick(() => {
        if (this.$refs.wordInput) this.$refs.wordInput.focus();
      });
    },
    ...
    checkWord() {
      ...
      this.input = '';
      this.$nextTick(() => {
        if (this.$refs.wordInput && !this.solved) this.$refs.wordInput.focus();
      });
    }
  }
};
...
.wordle-cell { 
  ...
  background: rgba(255,255,255,0.04); color: #94a3b8; border: 2px solid rgba(255,255,255,0.06);
  transition: all 0.3s;
}
...
.wordle-field { 
  ...
  font-family: 'Space Grotesk', sans-serif; letter-spacing: 4px; text-align: center;
}
.wordle-field:focus { border-color: #6366f1; box-shadow: 0 0 10px rgba(99,102,241,0.15); }
.wordle-check-btn { 
  width: 44px; display: flex; align-items: center; justify-content: center;
  background: rgba(99,102,241,0.2); border: 1px solid rgba(99,102,241,0.3);
  border-radius: 10px; color: #fff; cursor: pointer; font-weight: 700; font-size: 1.1rem;
  transition: all 0.2s;
}
...
.wordle-replay-btn { 
  ...
  border-radius: 8px; color: #cbd5e1; cursor: pointer; font-size: 0.75rem; font-weight: 600;

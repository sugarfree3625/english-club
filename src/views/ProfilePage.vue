    ...
    <div class="container">
      <div class="profile-page">
        ...
        <div class="profile-main">
          ...
          <template v-if="wordsLoading">
            <SkeletonLoader v-for="i in 5" :key="i" width="100%" height="60px" style="border-radius:14px;margin-bottom:8px" />
          </template>
          <template v-else>
...
export default {
  ...
  methods: {
    ...
    async loadAchievements() { try { const r = await axios.get('/api/achievements'); const d = (r.data || []).map(a => ({ ...a, progressPercent: a.progressPercent || (a.earned ? 100 : 0) })); const unlocked = d.filter(a => { const o = this.allAchievements.find(x => x.code === a.code); return o && !o.earned && a.earned; }); this.allAchievements = d; this.totalCount = d.length; this.earnedCount = d.filter(a => a.earned).length; if (unlocked.length && !this._shown) { this._shown = true; this.newAchievement = unlocked[0]; this.showConfetti = true; const xpResult = await addXP('achievement_earned'); if (xpResult) this.showXP?.(xpResult.xp_added, window.innerWidth / 2, window.innerHeight / 2); } } catch (e) {} },
    ...
    async loadMyHomework() { this.homeworkLoading = true; try { if (this.isTutor) this.allHomework = (await axios.get('/api/homework/all')).data || []; else if (this.isParent) this.allHomework = (await axios.get('/api/homework/children')).data || []; else this.allHomework = (await axios.get('/api/homework/my')).data || []; } catch(e) {} finally { this.homeworkLoading = false; } },
    openHomeworkTab(s) { this.hwStudent = s.id; this.tab = 'homework'; },
    async createHomework(formData) { try { await axios.post('/api/homework', formData, { headers: { 'Content-Type': 'multipart/form-data' } }); this.addToast('Задание создано! 📝', 'success'); await this.loadMyHomework(); } catch(e) { this.addToast(e.response?.data?.error || 'Ошибка', 'error'); } },
    async submitHomeworkAnswer({ answer }) { try { await axios.put(`/api/homework/${this.selectedHomework.id}`, { student_answer: answer, status: 'submitted' }); this.selectedHomework = null; this.addToast('Отправлено! 📤', 'success'); const xpResult = await addXP('homework_submitted'); if (xpResult?.leveled_up) this.addToast(`🎉 Уровень повышен до ${xpResult.level}!`, 'success'); if (xpResult) this.showXP?.(xpResult.xp_added, window.innerWidth / 2, window.innerHeight / 2); } catch { this.addToast('Ошибка', 'error'); } },
    async submitHomeworkGrade({ grade, comment }) { try { await axios.put(`/api/homework/${this.selectedHomework.id}`, { grade, teacher_comment: comment, status: 'completed' }); this.selectedHomework = null; this.addToast('Оценено! ⭐', 'success'); const xpResult = await addXP('homework_graded'); if (xpResult) this.showXP?.(xpResult.xp_added, window.innerWidth / 2, window.innerHeight / 2); } catch { this.addToast('Ошибка', 'error'); } },
    ...
    async returnHomework({ comment }) { try { await axios.put(`/api/homework/${this.selectedHomework.id}`, { teacher_comment: comment || 'Требуется доработка', status: 'in_progress' }); this.selectedHomework = null; this.addToast('🔄 Возвращено. Фидбек создан!', 'info'); await this.loadMyHomework(); } catch(e) { this.addToast('Ошибка', 'error'); } },
    ...
    async doBindParent(pid) { try { await axios.post('/api/parent/bind', { student_id: this.bindStudentId, parent_id: pid }); this.showBindParent = false; this.addToast('Привязан! ✅', 'success'); } catch(e) { this.addToast(e.response?.status === 409 ? 'Уже существует' : 'Ошибка', e.response?.status === 409 ? 'info' : 'error'); if (e.response?.status === 409) this.showBindParent = false; } },
    ...
    async uploadAvatar(e) { const f = e.target.files[0]; if (!f || f.size > 5*1024*1024) { this.addToast('Файл > 5MB', 'error'); return; } try { const fd = new FormData(); fd.append('img', f); const r = await axios.post('/api/nimg', fd); if (r.data.url) { await axios.put('/api/me', { avatar_url: r.data.url }); this.user.avatar_url = r.data.url; this.$emit('update-user', {...this.user, avatar_url: r.data.url}); this.addToast('Фото обновлено! 📸', 'success'); } } catch {} }
  }
};
</script>

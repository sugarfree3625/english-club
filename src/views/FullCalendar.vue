    ...
    <div v-if="view==='week'" class="week-view">
      <div class="week-scroll">
        <div class="week-grid" ref="weekGrid">
          ...
          <div class="whead" v-for="d in weekDaysList" :key="d.date" :class="{today:d.isToday}">
            <span>{{d.name}}</span><span class="wdate">{{d.dateStr}}</span>
          </div>
          ...
          <div class="events-layer">
            <div v-for="ev in positionedEvents" :key="ev.id" class="event-chip" :style="{background:ev.color,...ev._style}" @mousedown="startDrag($event,ev)" @click.stop="openPopover($event,ev)">
...
export default {
  ...
  data() {
    return {
      ...
      dragEv: null, resizeEv: null, dragSY:0, dragSX:0, dragOrig:null
    };
  },
  computed: {
    ...
    weekDaysList() {
      ...
      m.setDate(t.getDate()-(t.getDay()||7)+1+this.curWeek*7);
  ...
  methods: {
    async loadEvents() {
      try {
        ...
        this.events=(data||[]).map(s=>({...s,_time:this.fmtTime(s.start_time),_student:s.users?.username||'',color:s.color||this.defColor(s.lesson_type)}));
      } catch(e) { this.loadMockEvents(); }
    },
    loadMockEvents() {
      const now=new Date(), y=now.getFullYear(), m=now.getMonth();
      const mock=[];
      for(let i=0;i<15;i++){
        const d=new Date(y,m,1+Math.floor(Math.random()*28));
        const h=8+Math.floor(Math.random()*12);
        const start=new Date(d.getFullYear(),d.getMonth(),d.getDate(),h,Math.random()>0.5?30:0);
        const end=new Date(start.getTime()+(30+Math.floor(Math.random()*4)*30)*60000);
        const types=['online','offline','group'];
        const type=types[Math.floor(Math.random()*3)];
        mock.push({id:i+1,title:`Занятие ${i+1}`,start_time:start.toISOString(),end_time:end.toISOString(),lesson_type:type,color:this.defColor(type),notes:'',student_id:null,_time:this.fmtTime(start.toISOString()),_student:'Ученик'});
      }
      this.events=mock;
    },
    ...
    onDayClick(day) {
      ...
      if (this.getDayEvents(day.date).length === 0 && this.isTutor) {
        this.quickForm = { date: day.date, title: '' };
      }
    },
    async saveQuick() {
      if (!this.quickForm.title) return;
      const start = new Date(this.quickForm.date+'T10:00:00');
      const end = new Date(start.getTime()+30*60000);
      try {
        await axios.post('/api/slots',{title:this.quickForm.title,lesson_type:'online',start_time:start.toISOString(),end_time:end.toISOString(),color:'#10b981',notes:'',student_id:null});
        this.quickForm.date = null;
    ...
    openPopover(e, ev) {
      this.popover = { event: ev, x: e.clientX-10, y: e.clientY-10 };
    },
    onClickOutside() {
      if (this.popover.event) this.popover.event = null;
    },

    createAt(date, h, m) {
      if (!this.isTutor) return;
      this.modal = { show: true, event: null, form: { student_id:'',type:'online',title:'',date,time:`${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`,duration:30,notes:'',color:'#10b981',repeat:'none',repeat_count:4 } };
    },
    openModal(ev) {
      if (ev) {
        const sd = new Date(ev.start_time);
        this.modal = { show: true, event: ev, form: { student_id:ev.student_id||'',type:ev.lesson_type||'online',title:ev.title||'',date:sd.toISOString().split('T')[0],time:this.fmtTime(ev.start_time),duration:Math.round((new Date(ev.end_time)-sd)/60000)||30,notes:ev.notes||'',color:ev.color||'#6366f1',repeat:ev.repeat||'none',repeat_count:ev.repeat_count||4 } };
      } else {
        this.modal = { show: true, event: null, form: { student_id:'',type:'online',title:'',date:new Date().toISOString().split('T')[0],time:'10:00',duration:30,notes:'',color:'#10b981',repeat:'none',repeat_count:4 } };
      }
    },
    closeModal() { this.modal.show = false; this.modal.event = null; },
    async saveEvent() {
      try {
        const [h,m] = this.modal.form.time.split(':');
        const s = new Date(this.modal.form.date); s.setHours(+h,+m,0,0);
        const dur = Math.max(+this.modal.form.duration||30,30);
        const e = new Date(s.getTime()+dur*60000);
        const data = { title:this.modal.form.title, lesson_type:this.modal.form.type, student_id:this.modal.form.student_id||null, start_time:s.toISOString(), end_time:e.toISOString(), notes:this.modal.form.notes, color:this.modal.form.color, repeat:this.modal.form.repeat, repeat_count:this.modal.form.repeat_count };
        if (this.modal.event) await axios.put(`/api/slots/${this.modal.event.id}`,data);
        else await axios.post('/api/slots',data);
        this.closeModal(); this.addToast?.('✅ Сохранено','success'); this.loadEvents();
      } catch(e) { this.addToast?.('Ошибка','error'); }
    },
    async deleteEvent(ev) {
      if (!confirm('Удалить?')) return;
      try { await axios.delete(`/api/slots/${ev.id}`); this.loadEvents(); this.addToast?.('🗑 Удалено','info'); } catch(e) { this.addToast?.('Ошибка','error'); }
    },

    startDrag(e, ev) { if(!this.isTutor)return; e.preventDefault(); this.dragEv=ev; this.dragSX=e.clientX; this.dragSY=e.clientY; this.dragOrig={...ev}; },
    startResize(e, ev) { if(!this.isTutor)return; e.preventDefault(); this.resizeEv=ev; this.dragSY=e.clientY; this.dragOrig={...ev}; },
    onDrag(e) {
      ...
      if(this.resizeEv){
        const dy=e.clientY-this.dragSY, dm=Math.round(dy/mp/15)*15;
        const oe=new Date(this.dragOrig.end_time); let ne=new Date(oe.getTime()+dm*60000);
        if(ne-new Date(this.dragOrig.start_time)<30*60000)ne=new Date(new Date(this.dragOrig.start_time).getTime()+30*60000);
        this.resizeEv.end_time=ne.toISOString();
      }
    },
    async onDrop() {
      ...
      try {
        if(this.dragEv) await axios.put(`/api/slots/${ev.id}/move`,{start_time:ev.start_time,end_time:ev.end_time});
        else await axios.put(`/api/slots/${ev.id}`,{start_time:ev.start_time,end_time:ev.end_time});
      } catch(e) {}
...
.events-layer { position: absolute; top: 28px; left: 0; right: 0; bottom: 0; pointer-events: none; }

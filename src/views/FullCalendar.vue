...
export default {
  ...
  methods: {
    ...
    editSlot(slot) { 
      ...
      this.editingSlot = slot; const sd = new Date(slot.start_time);
      if (!slot.meeting_link && (slot.lesson_type === 'online' || slot.lesson_type === 'group-online')) {
        slot.meeting_link = 'https://meet.jit.si/english-club-' + slot.id;
      }
    ...
    async saveSlot() {
      try {
        ...
        const start = new Date(this.slotForm.date);
        start.setHours(parseInt(h), parseInt(m), 0, 0);
        const duration = Math.max(parseInt(this.slotForm.duration) || 30, 30);

  ...
  <div class="chat-container" :class="{ dark: isDark }" v-if="user?.id">
    <div class="chat-sidebar" :class="{ 'mobile-hidden': !showSidebar && activeChat }">
      ...
      <div class="dialog-list">
        <div v-for="d in dialogs" :key="d.partner_id" class="dialog-item" :class="{ active: activeChat === d.partner_id }" @click="openChat(d)">
          <img :src="d.avatar_url || 'https://ui-avatars.com/api/?name=' + d.username" class="dialog-avatar">
          <div class="dialog-info"><strong>{{ d.username }}</strong><small>{{ d.last_msg?.substring(0, 30) }}</small></div>
        </div>
...
.search-item:hover { background: #eef0ff; } .dark .search-item:hover { background: #334155; }

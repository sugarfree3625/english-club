...
(async () => {
  ...
  app.get('/api/users', auth, async (req, res) => { const q = (req.query.q || '').toLowerCase(); const uid = req.session.userId; const { data: u } = await supabase.from('users').select('id,username,avatar_url,level,rating,role').neq('id', uid); if (!u) return res.json([]); let filtered = u; if (req.session.role === 'admin' || req.session.role === 'host') {
  filtered = u; // админ видит всех
} else {
  filtered = u.filter(x => x.role === 'admin' || x.role === 'host'); // ученик видит только учителей
} if (q.length >= 1) filtered = filtered.filter(x => x.username?.toLowerCase().includes(q)); res.json(filtered.slice(0, 50)); });
